...
function typeTextSequentially(elements, index, section = null) {
  ...
  // Start typing after a short delay
  setTimeout(typeChar, index === 0 ? 300 : 500);
}

function typeText(textElement, highlightElement = null) {
  const fullText = textElement.dataset.text || "";
  if (!fullText) {
    console.warn("No text found in data-text attribute");
    return;
  }
  
  // Calculate final height before typing starts to prevent vertical shifting
  // Temporarily set the full text to measure its height
  const originalContent = highlightElement ? highlightElement.textContent : textElement.textContent;
  if (highlightElement) {
    highlightElement.textContent = fullText;
  } else {
  ...
  // Set min-height to prevent vertical shifting during typing
  textElement.style.minHeight = `${finalHeight}px`;
...
function launchHearts() {
  const layer = document.querySelector(".global-heart-layer");
  if (!layer) return;
  ...
  for (let i = 0; i < heartCount; i++) {
    ...
    const fallDistance = layerHeight + 180 + Math.random() * 140;
    const endScale = (Math.random() * 0.9) + 0.7;
    const startScale = Math.max(0.35, endScale - 0.45);
    const size = 18 + Math.random() * 26;
    const rotation = (Math.random() * 50) - 25;

    heart.style.left = `${startX}px`;
    heart.style.setProperty("--h-drift", `${driftX}px`);
    heart.style.setProperty("--h-fall", `${fallDistance}px`);
    heart.style.setProperty("--hscale", endScale.toFixed(2));
    heart.style.setProperty("--hscale-start", startScale.toFixed(2));
    heart.style.setProperty("--hrot", `${rotation}deg`);
    heart.style.setProperty("--hsize", `${size}px`);
    heart.style.animationDelay = `${i * 0.08}s`;

    const gradientId = `heartGradient${Date.now()}${Math.round(Math.random() * 1000)}${i}`;
    heart.innerHTML = `
      <svg viewBox="0 0 32 29" xmlns="http://www.w3.org/2000/svg">
        <path fill="url(#${gradientId})" d="M23.6,0c-2.5,0-4.8,1.2-6.3,3.2C15.2,1.2,12.9,0,10.4,0C4.7,0,0,4.7,0,10.4 c0,9.3,16.3,18.6,16.3,18.6S32.6,19.7,32.6,10.4C32.6,4.7,27.9,0,22.2,0z"/>
        <defs>
          <linearGradient id="${gradientId}" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stop-color="#ff8fa3" stop-opacity="0.85" />
            <stop offset="100%" stop-color="#ff6b81" stop-opacity="0.8" />
          </linearGradient>
        </defs>
      </svg>
    `;
...
// ---- GAMIFICATION SECTION SCALING ------------------------------------
// Similar to rankedmeetingmethods.html scaling approach
const GAMIFICATION_BASE_WIDTH = 1200;
const GAMIFICATION_BASE_HEIGHT = 682.5; // 1200 / (1600/910) = 682.5 to match exact card aspect ratio

function resizeGamification() {
  const wrapper = document.querySelector(".gamification-scale-wrapper");
  const root = document.querySelector(".gamification-root");
  
  if (!wrapper || !root) return;
  
  const rect = wrapper.getBoundingClientRect();
  // Scale to fill width, ensuring we don't exceed height
  const scaleX = rect.width / GAMIFICATION_BASE_WIDTH;
  const scaleY = rect.height / GAMIFICATION_BASE_HEIGHT;
...
if (typeof ResizeObserver !== "undefined") {
  const gamificationRO = new ResizeObserver(() => {
    resizeGamification();
  });
  
  // Observe the wrapper when it exists
  const checkGamification = () => {
    const wrapper = document.querySelector(".gamification-scale-wrapper");
    if (wrapper) {
      gamificationRO.observe(wrapper);
      resizeGamification();
    }
  };
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', checkGamification);
  } else {
    checkGamification();
  }
  
  // Also check when sections become visible (in case of lazy loading)
  const gamificationObserver = new MutationObserver(() => {
    const wrapper = document.querySelector(".gamification-scale-wrapper");
    if (wrapper && !wrapper.dataset.observed) {
  ...
  gamificationObserver.observe(document.body, {
    childList: true,
    subtree: true
  });
} else {
  window.addEventListener("resize", resizeGamification);
  resizeGamification();
}
